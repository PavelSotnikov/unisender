...
// версия 1.0.0  - 29.04.2020 - создание обработки
// версия 1.0.1  - 06.05.2020 - исправление ошибки реквизиты строк ограниченной длины из-за чего они обрезались при отправке на сервер
//							  - исключение пустых строк
//							  - отправка на сервер методом importContacts пакетами
...
Функция importContacts(ТаблицаАдресов, overwrite_lists = Ложь) Экспорт
	...
	Пока СтрокаНачала < ТаблицаАдресов.Количество() Цикл		
		СтрокаЗапросаНаСервер	= СтрокаЗапросаImportContacts(ТаблицаАдресов, overwrite_lists, СтрокаНачала, РазмерПорции);
...
Функция СтрокаЗапросаImportContacts(ТаблицаАдресов, overwrite_lists = Ложь, НачатьСоСтроки = 0, РазмерПорции = 400)
